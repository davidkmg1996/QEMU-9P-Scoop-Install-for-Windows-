...
/*
 * Windows equivalent of POSIX futimens(2): set mtime/atime on an open fd.
 * Implemented with SetFileTime on the fd's underlying HANDLE.
 */
int qemu_futimens_win32(int fd, const struct timespec times[2])
{
    ...
    LARGE_INTEGER li;
    const LONGLONG EPOCH_DIFF = 116444736000000000LL; /* 1601->1970 in 100ns */
    ...
    li.QuadPart = times[0].tv_sec * 10000000LL + times[0].tv_nsec / 100
                  + EPOCH_DIFF;
    atime_ft.dwLowDateTime = li.LowPart;
    atime_ft.dwHighDateTime = li.HighPart;
    li.QuadPart = times[1].tv_sec * 10000000LL + times[1].tv_nsec / 100
                  + EPOCH_DIFF;
    mtime_ft.dwLowDateTime = li.LowPart;
    mtime_ft.dwHighDateTime = li.HighPart;

    if (!SetFileTime(hFile, NULL, &atime_ft, &mtime_ft)) {
        errno = EINVAL;
        return -1;
    }
    return 0;
